...
public:
		...
		Vector3f& dPos, Vector3f& dSpeed, Vector3f& dRot) const
	{
		...
		//Koeffizienten abh‰ngig von den Materialien
		...
		//abh‰ngig von Reibung Geschwindigkeit aus Winkelgeschwindigkeit bestimmen
		...
		Vector3f& dPos, Vector3f& dSpeed, Vector3f& dRot) const
	{
		...
		//neue Parallelgeschwindigkeit abh‰ngig von Reibung
		...
		Sphere& s1, const Sphere& s2) const
	{
		...
		//Teile der Geschwindigkeiten, die senkrecht zur Ber¸hrebene liegen
		...
		//Teile der Rotationsgeschwindigkeiten, die senkrecht zur Ber¸hrebene liegen
...
private:
	CUDA_CALLABLE_MEMBER Vector3d correctPosition(const Sphere& s, const Plane& p) const{
		//Position der Kugel sollte auf jeden Fall vollst‰ndig "diesseits"
		//der Ebene bleiben. Dazu einfach die neue Position der Kugel 
		//korrigieren, indem senkrecht zur Ebene zur¸ckgesetzt wird
		f64 d = p.orientatedDistanceTo(s.x);

		//in Abh‰ngigkeit von der Seite, auf der sich der Mittelpunkt befindet,
